...
//Ashok starts here
// Macro to add per-trace statistics
#define OPT_TRACE_STATS_ADD(trace_length, jit_size) do { if (_Py_stats) _PyStats_AddTraceStats(trace_length, jit_size); } while(0)
//Ashok until here
...
//Ashok starts here
// function declarations for per-trace stats management
PyAPI_FUNC(int) _PyStats_AddTraceStats(uint32_t trace_length, size_t jit_size);
PyAPI_FUNC(void) _PyStats_ClearTraceStats(void);
PyAPI_FUNC(void) _PyStats_FreeTraceStats(void);
//Ashok until here
#else
...
//Ashok starts here
#define OPT_TRACE_STATS_ADD(trace_length, jit_size) ((void)0)
//Ashok until here
#endif  // !Py_STATS
