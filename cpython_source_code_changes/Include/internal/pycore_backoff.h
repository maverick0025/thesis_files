...
make_backoff_counter(uint16_t value, uint16_t backoff)
{
    ...
    //Ashok starts here
    // assert(value <= 0xFFF);
    assert(value <= 0xFFFF); //2^16 - 1 => max threshold Ashok inserts via command line
    //Ashok until here
...
//Ashok Starts here
PyAPI_DATA(int) _Py_JumpBackwardInitialValue;
PyAPI_DATA(int) _Py_JumpBackwardInitialBackoff;
PyAPI_DATA(int) _Py_SideExitInitialValue;
PyAPI_DATA(int) _Py_SideExitInitialBackoff;
PyAPI_DATA(int) _Py_AdaptiveCoolDownValue;
PyAPI_DATA(int) _Py_MaxChainDepth;
PyAPI_DATA(int) _Py_ConfidenceLowerLimit;

static inline _Py_BackoffCounter
initial_jump_backoff_counter(void)
{
    return make_backoff_counter(_Py_JumpBackwardInitialValue, _Py_JumpBackwardInitialBackoff);
}
...
{
    return make_backoff_counter(_Py_SideExitInitialValue, _Py_SideExitInitialBackoff);
}

//JUMPBACKWARDINITIALVALUE, BACKOFF, SIDE EXIT AND BACKOFFS
//Ashok until here with some changes
