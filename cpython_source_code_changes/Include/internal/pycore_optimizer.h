...
typedef struct {
    ...
    uint8_t linked:1;
    //Ashok starts here
    uint8_t chain_depth:8;  // Must be big enough for MAX_CHAIN_DEPTH - 1 i.e I ask the compiler to set 8 bits aside for this chain_depth. so max chaindepth i can have is 255 (2^8 -1)
...
It is a compile‐time constant that defines the maximum size (in number of symbolic slots) for the 
“abstract interpreter” memory structure that the optimizer uses when analyzing a trace. 
More concretely, it bounds the size of an array used within the _Py_UOpsContext structure:
*/
