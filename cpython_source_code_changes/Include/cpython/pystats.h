...
typedef struct _optimization_stats {
    ...
    //Ashok starts here
    // per-trace statistics (dynamic arrays)
    struct {
        uint32_t *trace_lengths; // Array of trace lengths (uop count)
        size_t *jit_sizes;      //Array of JIT code sizes (bytes)
        size_t count;           // number of traces stored
