...
//Ashok starts here
static void print_per_trace_stats(FILE *out, OptimizationStats *stats){
    if(stats->per_trace_stats.count == 0){
        fprintf(out, "No per-trace statistics available. \n");
    }

    fprintf(out, "\n Per-trace JIT statistics: \n");
    fprintf(out, " ========================= \n");
    fprintf(out, "Total traces compiled: %zu\n", stats->per_trace_stats.count);

    //Print individual trace stats
    for(size_t i = 0; i<stats->per_trace_stats.count; i++ ){
        fprintf(out, "Trace %zu: length = %u uops | JIT size = %zu bytes\n", 
                            i+1, 
                            stats->per_trace_stats.trace_lengths[i],
                            stats->per_trace_stats.jit_sizes[i]);
    }
    ...
    if (stats->per_trace_stats.count > 0) {
        ...
        for (size_t i = 0; i < stats->per_trace_stats.count; i++) {
            uint32_t length = stats->per_trace_stats.trace_lengths[i];
            size_t size = stats->per_trace_stats.jit_sizes[i];
        ...
        fprintf(out, "\nSummary:\n");
        fprintf(out, "--------\n");
        fprintf(out, "Average trace length: %.2f uops\n", 
                (double)total_length / stats->per_trace_stats.count);
        fprintf(out, "Average JIT size: %.2f bytes\n", 
                (double)total_size / stats->per_trace_stats.count);
        fprintf(out, "Trace length range: %u - %u uops\n", min_length, max_length);
        fprintf(out, "JIT size range: %zu - %zu bytes\n", min_size, max_size);
        fprintf(out, "Total JIT memory: %zu bytes\n", total_size);
        fprintf(out, "Ashok was here!\n");
    }
}
...
print_optimization_stats(FILE *out, OptimizationStats *stats)
{
    ...
    //Ashok starts here
    print_per_trace_stats(out, stats);
    //Ashok until here
}
#endif
...
//Ashok starts here
//Implement array mangement functions
#ifdef Py_STATS

#define INITIAL_TRACE_CAPACITY 64
#define TRACE_GROWTH_FACTOR 2

int _PyStats_AddTraceStats(uint32_t trace_length, size_t jit_size){

    if(!_Py_stats){
        return 0;
    }

    OptimizationStats *stats = &_Py_stats->optimization_stats;

    //Initialize arrays if needed
    if(stats->per_trace_stats.capacity == 0){
        stats->per_trace_stats.trace_lengths = PyMem_Calloc(INITIAL_TRACE_CAPACITY, sizeof(uint32_t));
        stats->per_trace_stats.jit_sizes = PyMem_Calloc(INITIAL_TRACE_CAPACITY, sizeof(size_t));

        if(!stats->per_trace_stats.trace_lengths || !stats->per_trace_stats.jit_sizes){
            _PyStats_FreeTraceStats();
            return -1;
        }

        stats->per_trace_stats.capacity = INITIAL_TRACE_CAPACITY;
        stats->per_trace_stats.count = 0;
    }

    // Grow arrays if needed
    if(stats->per_trace_stats.count >= stats->per_trace_stats.capacity){
        size_t new_capacity = stats->per_trace_stats.capacity * TRACE_GROWTH_FACTOR;
        
        uint32_t *new_trace_lengths = PyMem_Realloc(stats->per_trace_stats.trace_lengths,
                                                    new_capacity * sizeof(uint32_t));

        size_t *new_jit_sizes = PyMem_Realloc(stats->per_trace_stats.jit_sizes,
                                                    new_capacity * sizeof(uint32_t));
        
        if(!new_trace_lengths || !new_jit_sizes){
            PyMem_Free(new_trace_lengths);
            PyMem_Free(new_jit_sizes);
            return -1;
        }

        stats->per_trace_stats.trace_lengths = new_trace_lengths;
        stats->per_trace_stats.jit_sizes = new_jit_sizes;
        stats->per_trace_stats.capacity = new_capacity;
    }

    //store trace statistics

    size_t index = stats->per_trace_stats.count++;
    stats->per_trace_stats.trace_lengths[index] = trace_length;
    stats->per_trace_stats.jit_sizes[index] = jit_size;

    return 0;
}

void _PyStats_ClearTraceStats(void){

    if(!_Py_stats){
        return;
    }

    _Py_stats->optimization_stats.per_trace_stats.count = 0;
}

void _PyStats_FreeTraceStats(void){
    ...
    PyMem_Free(stats->per_trace_stats.trace_lengths);
    PyMem_Free(stats->per_trace_stats.jit_sizes);

    stats->per_trace_stats.trace_lengths = NULL;
    stats->per_trace_stats.jit_sizes = NULL;
    stats->per_trace_stats.capacity = 0;
    stats->per_trace_stats.count = 0;
}
