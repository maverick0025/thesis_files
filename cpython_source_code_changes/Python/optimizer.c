...
{
    // printf("at _PyOptmizer_Optimize to translate an instruction trace into sequence of micro-ops.Check the *frame pointer to get trace");
    ...
    for (;;) {
        ...
        switch (opcode) {
            ...
            case POP_JUMP_IF_TRUE:
            {
                ...
                //Ashok starts here
                if (confidence < _Py_ConfidenceLowerLimit) {
                    DPRINTF(2, "Confidence too low (%d < %d)\n", confidence, _Py_ConfidenceLowerLimit);
                    OPT_STAT_INC(low_confidence);
                    goto done;
                }
                //Ashok until here (need to modify into proper macro Confidence Lower Limit if needed)
