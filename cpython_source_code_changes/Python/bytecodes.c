       ...
       op(_GUARD_BINARY_OP_EXTEND, (descr/4, left, right -- left, right)) {
        ...
        tier1 op(_SPECIALIZE_JUMP_BACKWARD, (--)) {
            ...
            if (this_instr->op.code == JUMP_BACKWARD) {
                printf("Enabling specialization at family JUMP_BACKWARD");
...
// invalid by the time we actually try to fetch the item.
        ...
        tier2 op(_CHECK_FUNCTION_VERSION_INLINE, (func_version/2, callable_o/4 --)) {
            printf("tier2 checking function version in line");
        ...
        tier2 op(_EXIT_TRACE, (exit_p/4 --)) {
            ...
            if (exit->executor == NULL) {
                ...
                if (target->op.code == ENTER_EXECUTOR) {
                    printf("op code is ENTER_EXECUTOR. Now, that executor is invoked, instead of jumping to the uop interpreter at tier2_dispatch, te executor runs the function that ji_code points to.");
