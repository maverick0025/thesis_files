...
// Ashok starts here
    ...
    /* -X jit-th-param=<value>*/
    const wchar_t *jit_param = config_get_xoption_value(config, L"jit-th-param");
    if (jit_param != NULL) {
        int param = -1;

        //pass a single jit parameter value in command line and make necessary changes in here below mapping all other related values.
        if (swscanf(jit_param, L"%d", &param) == 1) {
            /* param value will be one of [0, 1, 2,...,16] 
            backoff values will be param directly.
            intial values will be 2^param
            */           
            if (param >= 0 && param < 17) {
                if(param == 0){
                    //backoff value will be 1 when if the threshold we wanna set it to 1. Because it doesn't make sense to make it 0
                    _Py_JumpBackwardInitialBackoff = 1; 
                    _Py_SideExitInitialBackoff = 1 ;
                }else{
                    _Py_JumpBackwardInitialBackoff = param;
                    _Py_SideExitInitialBackoff = param;
                }
                
                //adaptive cool down value should always be atleast 1 greater than the backoff value. Preferrably the nearest prime number + 1
                
                if (param <= 4) {
                    if (param == 0) {
                        _Py_AdaptiveCoolDownValue = 2;  // backoff=1, need >1
                    } else if (param == 1) {
                        _Py_AdaptiveCoolDownValue = 2;  // backoff=1, need >1  
                    } else if (param == 2) {
                        _Py_AdaptiveCoolDownValue = 3;  // backoff=2, need >2
                    } else if (param == 3) {
                        _Py_AdaptiveCoolDownValue = 5;  // backoff=3, need >3, use prime
                    } else if (param == 4) {
                        _Py_AdaptiveCoolDownValue = 7;  // backoff=4, need >4, use prime
                    }
                } else if(param < 12){
                    // For param >= 5 until param 12 the def, use 17 (prime number)
                    _Py_AdaptiveCoolDownValue = 17;
                } else{
                    _Py_AdaptiveCoolDownValue = 52;
                }

                // if(param <5){
                //     _Py_AdaptiveCoolDownValue = 18; 
                // }else{
                //     _Py_AdaptiveCoolDownValue = 52;
                // }

                _Py_JumpBackwardInitialValue = (1<<param) - 1;
                _Py_SideExitInitialValue = (1<<param) - 1;

                /* CHAIN DEPTH
                I asked the compiler to set aside 8 bits (max value) for this chain depth. check it in pycore_optimizer.h file in _PyVMData structure.
                If in future, I ought to increase chain depth, it's not possible. might need to change the entire _PyVMData datastructure
                */
               //I might have to invert this setting of max chain depth reverse polarities
                if(param <= 8){
                    _Py_MaxChainDepth = 254; //max chain depth possible is 255
                }else if(param < 12){
                    _Py_MaxChainDepth = 16;
                }
                else if(param >= 12){
                    _Py_MaxChainDepth = 4;
                }else{
                    // _Py_MaxChainDepth = 4*(4^((param-12)/2)) => 2^param-10;
                    int helperChainDepthCal = param-10;
                    _Py_MaxChainDepth = (1<<helperChainDepthCal);
                }

                /*
                Confidence mechanism
                default lower limit is 333.
                */
               if(param == 0){
                    _Py_ConfidenceLowerLimit = 1;
               }
               else if(param <=4 ){
                    _Py_ConfidenceLowerLimit =  50;
               }
               else if(param <=8){
                    _Py_ConfidenceLowerLimit =  100;
                }
               else if(param < 12){
                    _Py_ConfidenceLowerLimit = 200;
               }else if (param == 12){
                    _Py_ConfidenceLowerLimit = 333;
               }else{
                    _Py_ConfidenceLowerLimit = 333;
               }
            
            }else{
                return _PyStatus_ERR("-X jit-th-param requires valid value. Choose from [0-16]");
            }
        
            if(param >=0){
                fprintf(stderr, "JIT Parameters Set:\n");
                fprintf(stderr, "  Threshold param: %d\n", param);
                fprintf(stderr, "  Jump backward initial: %d\n", _Py_JumpBackwardInitialValue);
                fprintf(stderr, "  Side exit initial: %d\n", _Py_SideExitInitialValue);
                fprintf(stderr, "  Backoff values: %d\n", _Py_JumpBackwardInitialBackoff);
                fprintf(stderr, "  Adaptive cooldown: %d\n", _Py_AdaptiveCoolDownValue);
                fprintf(stderr, "  Max Chain Depth: %d\n", _Py_MaxChainDepth);
                fprintf(stderr, "  Confidence Lower Limit: %d\n", _Py_ConfidenceLowerLimit);
            }
        }        
        else {
            return _PyStatus_ERR("-X jit-th-param requires N. Choose from [0-16]");
        }
    }
