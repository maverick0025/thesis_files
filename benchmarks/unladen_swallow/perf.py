...
def main(argv, bench_funcs=BENCH_FUNCS, bench_groups=BENCH_GROUPS):
    ...
    # stats1 = pypyjit.get_stats_snapshot()
    ...
    if len(errors) > 0:
        raise errors[-1]
    # XXX printing sometimes hangs on the benchmark runner
    # in the meantime, just return the results with no
    # printing
    return results
    print
    ...
    # stats2 = pypyjit.get_stats_snapshot()

    # Print specific attributes from the snapshots
    
    '''
    print("Stats before loop:")
    print(f"Counters: {stats1.counters}")
    print(f"Counter times: {stats1.counter_times}")

    print("--------------")
    print("Stats after loop:")
    print(f"Counters: {stats2.counters}")
    print(f"Counter times: {stats2.counter_times}")

    print("--------------")
    print("Differences:")
    print(f"Counters diff: {dict((k, stats2.counters.get(k, 0) - stats1.counters.get(k, 0)) for k in stats2.counters)}")
    print(f"Counter times diff: {dict((k, stats2.counter_times.get(k, 0) - stats1.counter_times.get(k, 0)) for k in stats2.counter_times)}")
    '''
